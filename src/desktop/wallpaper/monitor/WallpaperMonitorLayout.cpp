...
LayoutMode ParseLayoutMode(const std::wstring& value) noexcept {
    ...
    // Current product contract: one wallpaper selection may paint only one
    // display. Legacy span/clone/unknown config values therefore migrate to the
    // primary display instead of silently painting multiple monitors.
...
bool SelfTestMonitorLayoutGeometry() noexcept {
    ...
    // Keep raw geometry helpers tested for all enum values. Product config
    // parsing below is what prevents legacy Span/Clone from being activated.
    ...
    return ParseLayoutMode(L"clone") == LayoutMode::PrimaryOnly &&
           ParseLayoutMode(L"span") == LayoutMode::PrimaryOnly &&
           ParseLayoutMode(L"unknown") == LayoutMode::PrimaryOnly &&
           ParseLayoutMode(L"independent") == LayoutMode::Independent &&
