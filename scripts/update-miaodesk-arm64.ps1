param(
    [string]$Repo = "GoodLoongStudio/MiaoDesk"
)

$ErrorActionPreference = "Stop"
$ProgressPreference = "SilentlyContinue"

$ArtifactName = "MiaoDesk-Native-Search-ARM64"
$Workflow = "native-search-windows.yml"
$DeployDir = Join-Path $env:LOCALAPPDATA "MiaoDesk\NativeTest"
...
$MutexName = "Local\MiaoDeskArm64Updater"
...
function Materialize-Runtime([string]$Destination, [string]$BuildSha) {
    Step "Materializing changed MiaoDesk RuntimeBundle"
    if (-not (Get-Command git -ErrorAction SilentlyContinue)) { throw "Git was not found in PATH." }
    $runtimeRepo = Join-Path $env:TEMP ("MiaoDesk-RuntimeSource-" + [guid]::NewGuid().ToString("N"))
    try {
...
function Test-StagedPackage([string]$Root) {
    ...
    $search = Join-Path $Root "MiaoDesk.exe"; $wallpaper = Join-Path $Root "MiaoDeskWallpaper.exe"; $workbench = Join-Path $Root "MiaoDeskHarness.exe"
    ...
    Assert-File $search "MiaoDesk.exe"; Assert-File $wallpaper "MiaoDeskWallpaper.exe"; Assert-File $workbench "MiaoDeskHarness.exe"; Assert-File $node "bundled AI runtime"; Assert-File $workbenchCli "advanced workbench runtime"; Assert-File $agentCli "agent runtime"; Assert-File $goz "file index client"; Assert-File $gozd "file index service"
    Test-Binary $search "MiaoDesk"; Test-Binary $wallpaper "MiaoDesk Wallpaper"; Test-Binary $workbench "MiaoDesk Advanced Workbench"; Test-Binary $node "MiaoDesk Agent Runtime" @($agentCli, "--version"); Test-Binary $workbench "MiaoDesk Advanced Workbench smoke" @("--harness-smoke-test")
}
...
function Stop-DeployedProcesses {
    Step "Stopping currently installed MiaoDesk processes"
...
function Wait-IndexReady([string]$IndexExe) { Assert-File $IndexExe "installed file index client"; for ($i=0; $i -lt 120; $i++) { if ((Probe $IndexExe @("--status")) -eq 0) { return }; Start-Sleep -Milliseconds 500 }; throw "MiaoDesk file index service did not become reachable after installation." }
function Should-ShowWallpaperSettings { $config = Join-Path $env:LOCALAPPDATA "MiaoDesk\wallpaper.ini"; if (-not (Test-Path $config -PathType Leaf)) { return $true }; try { return -not [bool](Select-String -Path $config -Pattern "^Version=3$" -ErrorAction Stop) } catch { return $true } }
function Start-DeployedMiaoDesk([string]$Root) { $wallpaper = Join-Path $Root "MiaoDeskWallpaper.exe"; $search = Join-Path $Root "MiaoDesk.exe"; Assert-File $wallpaper "MiaoDeskWallpaper.exe"; Assert-File $search "MiaoDesk.exe"; if (Should-ShowWallpaperSettings) { Start-Process -FilePath $wallpaper -ArgumentList "--settings" } else { Start-Process -FilePath $wallpaper }; Start-Process -FilePath $search }
...
function Recover-InterruptedUpdate {
    ...
    Step "Recovering an interrupted MiaoDesk update"; $state = Get-Content $JournalPath -Raw | ConvertFrom-Json; if ([int]$state.schema -ne 1) { throw "Unsupported update recovery journal schema." }
...
try {
    ...
    if (-not $mutexHeld) { throw "Another MiaoDesk update is already running." }
    ...
    Step "Resolving current MiaoDesk main"; $mainSha = Invoke-GhText @("api", "repos/$Repo/commits/main", "--jq", ".sha"); Write-Host ("main: {0}" -f $mainSha) -ForegroundColor DarkGray
    ...
    if (Test-Path $InstalledBuildMarker -PathType Leaf) { $installedSha=([string](Get-Content $InstalledBuildMarker -Raw -ErrorAction SilentlyContinue)).Trim(); if ($installedSha -eq $validated.BuildSha) { Write-Host "`nMiaoDesk is already on the latest validated ARM64 build." -ForegroundColor Green; Write-Host ("Installed validated build: {0}" -f $validated.BuildSha) -ForegroundColor Green; return } }

    $work=Join-Path $env:TEMP ("MiaoDesk-Updater-"+[guid]::NewGuid().ToString("N")); $artifact=Join-Path $work "artifact"; $next=Join-Path $DeployParent ("NativeTest.next-"+[guid]::NewGuid().ToString("N")); New-Item -ItemType Directory -Force -Path $work,$next,$DeployParent | Out-Null
    ...
    Step "Installing validated MiaoDesk ARM64 package"
    if ($hadExistingInstall) { Move-Item -LiteralPath $DeployDir -Destination $previous -ErrorAction Stop }
    if (Test-Path $DeployDir) { throw ("MiaoDesk install directory still exists after backup move: {0}" -f $DeployDir) }
    ...
    Step "Running installed package self-tests"; Test-Binary (Join-Path $DeployDir "MiaoDesk.exe") "MiaoDesk"; Test-Binary (Join-Path $DeployDir "MiaoDeskWallpaper.exe") "MiaoDesk Wallpaper"; Test-Binary (Join-Path $DeployDir "MiaoDeskHarness.exe") "MiaoDesk Advanced Workbench"; Test-Binary (Join-Path $DeployDir "MiaoDeskHarness.exe") "MiaoDesk Advanced Workbench smoke" @("--harness-smoke-test")
    Step "Starting MiaoDesk"; Start-DeployedMiaoDesk $DeployDir; Remove-UpdateJournal; $swapStarted=$false
    ...
    Write-Host "`nMiaoDesk update completed successfully." -ForegroundColor Green; Write-Host ("Installed validated build: {0}" -f $validated.BuildSha) -ForegroundColor Green; Write-Host ("GitHub Actions run: {0}" -f $validated.RunId) -ForegroundColor DarkGray; Write-Host ("Install path: {0}" -f $DeployDir) -ForegroundColor DarkGray
} catch {
    $failure=$_.Exception.Message; Write-Host "`nMiaoDesk update failed." -ForegroundColor Red; Write-Host $failure -ForegroundColor Red
    if ($swapStarted) {
        try {
            Step "Rolling back MiaoDesk installation"; Stop-DeployedProcesses; Invoke-ElevatedIndexService (Join-Path $DeployDir "Goz\gozd.exe") "uninstall" -IgnoreFailure
            ...
            if ($hadExistingInstall -and (Test-Path $DeployDir -PathType Container)) { $restoredService=Join-Path $DeployDir "Goz\gozd.exe"; $restoredClient=Join-Path $DeployDir "Goz\goz.exe"; if (Test-Path $restoredService -PathType Leaf) { Invoke-ElevatedIndexService $restoredService "install"; if (Test-Path $restoredClient -PathType Leaf) { Wait-IndexReady $restoredClient } }; Start-DeployedMiaoDesk $DeployDir; Write-Host "Rollback completed. The previous MiaoDesk installation was restored." -ForegroundColor Green } else { Write-Host "Rollback completed. The failed package was removed." -ForegroundColor Yellow }
