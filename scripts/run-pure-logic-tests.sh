...
echo "        SceneD3D11Test        四个自测里两个已经搬走了(见 MiaoD3D11RenderPolicy.cpp),"
echo "                             剩下三个各自的理由不同,见 SceneD3D11Renderer.cpp 头部:"
echo "                             粒子那条的头文件就带 d3d11.h(挪不动)、TransformMath 是"
echo "                             文件局部符号、路径策略那条看着纯逻辑其实依赖'反斜杠是"
echo "                             路径分隔符'(POSIX 上会返回 false,本机实测过)。"
