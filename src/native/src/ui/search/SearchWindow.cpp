...
void ApplyWindowShape(HWND hwnd, bool expanded) {
    ...
    // The GDI region is deliberately a fraction less rounded than the antialiased D2D pill.
    // It only removes the rectangular HWND corners; it must never clip the D2D edge pixels.
    // The previous +1/57px region disagreed with the 720x56 / 28px D2D geometry and produced
    // the visible gray side-lobes on real ARM64 Windows.
    HRGN region = CreateRoundRectRgn(0, 0, kWindowWidth, height, 54, 54);
    if (!region) return;
...
void DrawSearchGlyph(ID2D1HwndRenderTarget* target, ID2D1Brush* brush) {
    ...
    target->DrawEllipse(D2D1::Ellipse(D2D1::Point2F(25.5f, 26.5f), 8.6f, 8.6f), brush, 1.6f);
    target->DrawLine(D2D1::Point2F(31.7f, 32.7f), D2D1::Point2F(37.4f, 38.4f), brush, 1.6f);
}
...
void DrawMicrophoneGlyph(ID2D1HwndRenderTarget* target, ID2D1Brush* brush) {
    ...
    target->DrawRoundedRectangle(
        D2D1::RoundedRect(D2D1::RectF(x - 4.1f, 15.6f, x + 4.1f, 31.0f), 4.1f, 4.1f), brush, 1.45f);
    target->DrawLine(D2D1::Point2F(x - 7.7f, 27.6f), D2D1::Point2F(x - 7.7f, 30.5f), brush, 1.45f);
    target->DrawLine(D2D1::Point2F(x + 7.7f, 27.6f), D2D1::Point2F(x + 7.7f, 30.5f), brush, 1.45f);
    target->DrawLine(D2D1::Point2F(x - 7.7f, 30.5f), D2D1::Point2F(x - 4.8f, 35.0f), brush, 1.45f);
    target->DrawLine(D2D1::Point2F(x + 7.7f, 30.5f), D2D1::Point2F(x + 4.8f, 35.0f), brush, 1.45f);
    target->DrawLine(D2D1::Point2F(x - 4.8f, 35.0f), D2D1::Point2F(x + 4.8f, 35.0f), brush, 1.45f);
    target->DrawLine(D2D1::Point2F(x, 35.0f), D2D1::Point2F(x, 40.0f), brush, 1.45f);
    target->DrawLine(D2D1::Point2F(x - 4.2f, 40.0f), D2D1::Point2F(x + 4.2f, 40.0f), brush, 1.45f);
}
...
void DrawSparkleGlyph(ID2D1Factory* factory, ID2D1HwndRenderTarget* target, ID2D1Brush* brush) {
    ...
    sink->AddBezier(D2D1::BezierSegment(D2D1::Point2F(x + 1.1f, y - 3.5f), D2D1::Point2F(x + 3.5f, y - 1.1f), D2D1::Point2F(x + 9.6f, y)));
    sink->AddBezier(D2D1::BezierSegment(D2D1::Point2F(x + 3.5f, y + 1.1f), D2D1::Point2F(x + 1.1f, y + 3.5f), D2D1::Point2F(x, y + 9.6f)));
    sink->AddBezier(D2D1::BezierSegment(D2D1::Point2F(x - 1.1f, y + 3.5f), D2D1::Point2F(x - 3.5f, y + 1.1f), D2D1::Point2F(x - 9.6f, y)));
    sink->AddBezier(D2D1::BezierSegment(D2D1::Point2F(x - 3.5f, y - 1.1f), D2D1::Point2F(x - 1.1f, y - 3.5f), D2D1::Point2F(x, y - 9.6f)));
    sink->EndFigure(D2D1_FIGURE_END_CLOSED);
    if (FAILED(sink->Close())) return;
    target->DrawGeometry(geometry.Get(), brush, 1.35f);
    target->DrawLine(D2D1::Point2F(x - 14.0f, y + 5.0f), D2D1::Point2F(x - 14.0f, y + 9.6f), brush, 0.95f);
    target->DrawLine(D2D1::Point2F(x - 16.3f, y + 7.3f), D2D1::Point2F(x - 11.7f, y + 7.3f), brush, 0.95f);
}
...
void SearchWindow::ApplyWindows11Style() {
    ...
    // DWM must not add its own rectangular material/border behind our pixel-owned pill.
    const int corner = 1; // DWMWCP_DONOTROUND
...
void SearchWindow::PositionWindow() {
    ...
    int y = positionLoaded_ ? std::clamp(savedY_, top, std::max(top, bottom - kCollapsedHeight))
                            : top + 22;
    if (!positionLoaded_) {
        savedX_ = x;
        savedY_ = y;
    ...
    SetWindowPos(hwnd_, nullptr, x, y, kWindowWidth,
                 expanded_ ? kExpandedHeight : kCollapsedHeight, SWP_NOZORDER | SWP_NOACTIVATE);
}
...
LRESULT CALLBACK SearchWindow::EditProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    if (message == WM_KEYDOWN) {
        if (wParam == VK_DOWN && !self->results_.empty()) {
            ...
            self->selected_ = std::min<int>(self->selected_ + 1,
                                            static_cast<int>(self->results_.size()) - 1);
...
void SearchWindow::OnQueryChanged() {
    ...
    SetExpanded(true);
    if (query.front() == L'/') {
        results_.push_back({ResultKind::Status, L"图灵智能桌面命令",
...
void SearchWindow::MergeResults() {
    ...
    if (!fileSearchAvailable_)
        results_.push_back({ResultKind::Status, L"文件搜索正在启动", L"文件索引服务暂不可用。", L"", -1000});
    else if (fileSearchQueryFailed_)
        results_.push_back({ResultKind::Status, L"文件查询失败", L"文件索引服务已连接，但本次查询没有成功。", L"", -1000});
...
void SearchWindow::ExecuteSelected(bool forceL3) {
    ...
    if (!forceL3 && selected_ >= 0 && selected_ < static_cast<int>(results_.size())) {
        ...
        if (IsLaunchable(result.kind) && !result.target.empty()) {
            const auto code = reinterpret_cast<INT_PTR>(ShellExecuteW(hwnd_, L"open", result.target.c_str(),
...
void SearchWindow::Draw() {
    if (!renderTarget_) {
        ...
        // The reference uses 45% white over a blurred light material. DWM backdrop is intentionally
        // disabled because it leaked rectangular corners on real Windows, so the self-owned layer
        // is slightly brighter to preserve the same perceived luminance over dark wallpapers.
        renderTarget_->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White, 0.76f), glassBrush_.GetAddressOf());
        renderTarget_->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White, 0.80f), hoverGlassBrush_.GetAddressOf());
        renderTarget_->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White, 0.82f), focusGlassBrush_.GetAddressOf());
        renderTarget_->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White, 0.84f), panelBrush_.GetAddressOf());
        renderTarget_->CreateSolidColorBrush(D2D1::ColorF(0x374151, 0.98f), textBrush_.GetAddressOf());
        renderTarget_->CreateSolidColorBrush(D2D1::ColorF(0x6B7280, 0.98f), secondaryBrush_.GetAddressOf());
        renderTarget_->CreateSolidColorBrush(D2D1::ColorF(0x5B9CFF, 0.11f), selectionBrush_.GetAddressOf());
        renderTarget_->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White, 0.68f), borderBrush_.GetAddressOf());
        renderTarget_->CreateSolidColorBrush(D2D1::ColorF(0x4285F4, 0.90f), accentBrush_.GetAddressOf());
    ...
    renderTarget_->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White, 0.86f), hoverOutline.GetAddressOf());
    renderTarget_->CreateSolidColorBrush(D2D1::ColorF(0x73A7FF, 0.90f), focusedOutline.GetAddressOf());
    renderTarget_->CreateSolidColorBrush(D2D1::ColorF(0x5B9CFF, 0.98f), activeOutline.GetAddressOf());
    renderTarget_->CreateSolidColorBrush(D2D1::ColorF(0x4285F4, 0.14f), softGlow.GetAddressOf());

    Microsoft::WRL::ComPtr<ID2D1GradientStopCollection> edgeStops;
    D2D1_GRADIENT_STOP stops[3] = {
        {0.0f, D2D1::ColorF(D2D1::ColorF::White, 0.92f)},
        {0.48f, D2D1::ColorF(D2D1::ColorF::White, 0.58f)},
        {1.0f, D2D1::ColorF(D2D1::ColorF::White, 0.18f)}
    };
    ...
    if (SUCCEEDED(renderTarget_->CreateGradientStopCollection(stops, 3, edgeStops.GetAddressOf())) && edgeStops) {
        const D2D1_LINEAR_GRADIENT_BRUSH_PROPERTIES edgeProps{
            D2D1::Point2F(0.0f, 0.0f), D2D1::Point2F(width, 56.0f)};
        renderTarget_->CreateLinearGradientBrush(edgeProps, edgeStops.Get(), edgeHighlight.GetAddressOf());
    }
    ...
    // One shared pixel-aligned geometry is used for fill and the primary outline. The 0.5px inset
    // keeps the 1px stroke entirely on device pixels and away from the coarse HWND region boundary.
    const auto bar = D2D1::RoundedRect(D2D1::RectF(0.5f, 0.5f, width - 0.5f, 55.5f), 27.5f, 27.5f);
    renderTarget_->FillRoundedRectangle(bar, fill);

    if (editFocused_ && softGlow) {
        const float glowInset = active ? 3.0f : 2.75f;
        const float glowWidth = active ? 2.6f : 1.9f;
        renderTarget_->DrawRoundedRectangle(
            D2D1::RoundedRect(D2D1::RectF(glowInset, glowInset, width - glowInset, 56.0f - glowInset),
    ...
    if (active && activeOutline) {
        mainOutline = activeOutline.Get();
        outlineWidth = 1.10f;
    } else if (editFocused_ && focusedOutline) {
        mainOutline = focusedOutline.Get();
        outlineWidth = 1.05f;
    } else if (hovered_ && hoverOutline) {
        mainOutline = hoverOutline.Get();
        outlineWidth = 1.0f;
    }
    renderTarget_->DrawRoundedRectangle(bar, mainOutline, outlineWidth);

    if (edgeHighlight) {
        const float inset = active ? 1.75f : editFocused_ ? 1.65f : hovered_ ? 1.55f : 1.5f;
        renderTarget_->DrawRoundedRectangle(
            D2D1::RoundedRect(D2D1::RectF(inset, inset, width - inset, 56.0f - inset),
    ...
    // Text and caret share the same DirectWrite layout/origin so they cannot drift apart.
    Microsoft::WRL::ComPtr<IDWriteTextLayout> queryLayout;
    if (!currentQuery_.empty()) {
        if (SUCCEEDED(writeFactory_->CreateTextLayout(
                currentQuery_.c_str(), static_cast<UINT32>(currentQuery_.size()), inputFormat_.Get(),
                static_cast<float>(kEditRight - kEditLeft), 56.0f, queryLayout.GetAddressOf())) && queryLayout) {
            renderTarget_->DrawTextLayout(D2D1::Point2F(static_cast<float>(kEditLeft), 0.0f),
                                          queryLayout.Get(), textBrush_.Get(), D2D1_DRAW_TEXT_OPTIONS_CLIP);
        }
    } else {
        ...
        renderTarget_->DrawText(placeholder, static_cast<UINT32>(std::size(placeholder) - 1),
                                inputFormat_.Get(),
                                D2D1::RectF(placeholderX, 0.0f, static_cast<float>(kEditRight), 56.0f),
    ...
    if (editFocused_ && caretVisible_) {
        ...
        if (queryLayout) {
            ...
            const UINT32 caretIndex = std::min<UINT32>(static_cast<UINT32>(selectionEnd),
                                                       static_cast<UINT32>(currentQuery_.size()));
            FLOAT hitX = 0.0f;
            FLOAT hitY = 0.0f;
            DWRITE_HIT_TEST_METRICS metrics{};
            if (SUCCEEDED(queryLayout->HitTestTextPosition(caretIndex, FALSE, &hitX, &hitY, &metrics))) {
                caretX += hitX;
                caretTop = hitY + 1.0f;
                caretBottom = hitY + std::max(18.0f, metrics.height - 1.0f);
            }
        }
        renderTarget_->DrawLine(D2D1::Point2F(caretX, caretTop),
                                D2D1::Point2F(caretX, caretBottom),
                                accentBrush_.Get(), 1.15f);
    }
    ...
    if (expanded_) {
        for (std::size_t i = 0; i < results_.size(); ++i) {
            ...
            renderTarget_->DrawText(title.c_str(), static_cast<UINT32>(title.size()), titleFormat_.Get(),
                                    D2D1::RectF(rowTextLeft, y + 4, width - 26, y + 28), textBrush_.Get(),
    ...
    if (hr == D2DERR_RECREATE_TARGET) {
        renderTarget_.Reset();
        glassBrush_.Reset();
        hoverGlassBrush_.Reset();
        focusGlassBrush_.Reset();
        panelBrush_.Reset();
        textBrush_.Reset();
