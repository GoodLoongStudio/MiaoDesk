...
public:
    ...
    std::vector<ApiProfile> Load() {
        ...
        for (const wchar_t* cursor = sections.data(); *cursor; cursor += wcslen(cursor) + 1) {
            ...
            profile.imageBaseUrl = ReadIni(path_, cursor, L"imageBaseUrl");
    ...
    void Save(const ApiProfile& profile) {
        ...
        WriteIni(path_, section.c_str(), L"imageBaseUrl", profile.imageBaseUrl);
...
struct PageState {
    ...
    ApiProfile FormProfile() const {
        ...
        profile.imageBaseUrl = Trim(WindowText(imageBaseUrl));
