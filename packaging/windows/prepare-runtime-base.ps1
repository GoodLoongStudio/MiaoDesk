...
$BundleRoot = Join-Path $RepoRoot ("runtime\{0}" -f $Architecture)
...
if (-not (Test-Path $LockPath -PathType Leaf)) { throw "Pinned Runtime lock is missing: $LockPath" }
...
Write-Host "$Architecture base Runtime ready: Node + npm + goz." -ForegroundColor Green
