...
- [x] Temporarily remove freeform Widget editing from the M3 product surface and ship three fixed showcase formats (`GlassClock`, `TodayTasks`, `WeatherGlass`) with preset-owned geometry.
...
- [x] Require the real-Windows acceptance executable to validate exactly one enabled `玻璃时钟`, `今日待办`, and `玻璃天气`, preset-owned sizes and same-monitor non-overlap before any durable phase cursor can advance.
...
- [x] Package `TuringDeskWidgetAcceptance.exe` in the compact ARM64 artifact and require installed acceptance to prove a valid installed build marker, ARM64 PE machine `0xAA64`, and checkout-HEAD/build-SHA identity before collecting evidence.
- [x] Keep the shared ARM64 RuntimeBundle outside per-build staging, reuse it through Junctions, and regression-test prompt-free Junction cleanup without traversing `node_modules`.
...
- the current beginner surface intentionally uses fixed presets only: `玻璃时钟`, `今日待办`, and `玻璃天气`; users can drag widgets on the desktop to reposition them with persisted coordinates, while resize/monitor-edit-mode work remains deferred until visible-runtime acceptance is proven;
- fixed preset placement scans normalized desktop space from the top-right, rejects candidates intersecting existing enabled Widgets plus the product gap, and falls back deterministically only when the display is too crowded to find a free slot; desktop drag updates normalized `x/y` through `WidgetService::Update` without changing preset-owned `width/height`;
- `scripts/verify-widget-product-model.ps1` guards the fixed-format contract, desktop drag persistence path, the strict three-widget acceptance set and config-v2 identity continuity, and is run by both exact-head x64 and ARM64 workflows;
- `TuringDeskWidgetAcceptance.exe` and the phase runner enforce `baseline -> settings -> search -> explorer -> monitor`, exactly one of each fixed showcase template at preset geometry without same-monitor overlap, same Widget identity/configuration, same interactive Windows session and the same acceptance binary;
- installed acceptance now rejects missing/malformed `.installed-build-sha`, rejects a non-ARM64 acceptance probe by PE machine, and rejects evidence collection when the installed build SHA differs from the checkout `HEAD`;
- the ARM64 updater keeps one stable shared `RuntimeBundle` and links `Runtime/Pi/Goz` into deployments; updater guards plus a real Windows temp-Junction regression test protect prompt-free cleanup and prevent deep-copy regressions;
...
- `docs/WIDGET_RUNTIME_HEALTH_M3.md`, `docs/WIDGET_PLACEMENT_HEALTH_M3.md`, `docs/WIDGET_ACCEPTANCE_SEQUENCE_M3.md`, `docs/WIDGET_ACCEPTANCE_EVIDENCE_M3.md`, `docs/WIDGET_PRODUCT_MODEL_M3.md` and `docs/WIDGET_INSTALLED_ACCEPTANCE_M3.md` describe the active health/product/evidence semantics and remaining acceptance work.

Real Windows acceptance flow:

```text
install/update the exact validated ARM64 main build
-> installed build SHA matches the acceptance checkout HEAD and acceptance probe reports ARM64 PE machine 0xAA64
-> create the three fixed desktop widget formats
-> exactly one of each fixed format is enabled at preset-owned geometry
-> all three are visibly rendered without overlap
-> drag a widget on the desktop and confirm the new position persists after release
...
M1 is closed. M2 implementation is physically centralized in `DesktopShellHost` and the implementation baseline `1ed59a6a9c270408024e7143302a45592d2156a1` passed both x64 and ARM64 Windows validation; its real-Windows wallpaper/Widget/icon layering and recovery acceptance remains an outstanding gate and is explicitly carried into M3 acceptance. M3 now has domain-owned per-surface process/HWND/parent/style/visibility, preferred-child Environment/Controller/Navigation telemetry, shared read-only desktop/shell z-order telemetry, combined rendering-health semantics, stable actionable issue codes, production Widget list health/action display, Pi per-surface issue/action reporting through the shared snapshot, observed Settings/Search/Explorer/monitor evidence, same-session and same-binary continuity, config-v2 service-routed fixed showcase identity/placement continuity sealed into the final evidence manifest, Settings/Search acceptance pinned to the expected visible product class/process in the same session, strict three-widget plus reported-and-ready lifecycle preconditions that are failure-atomic with respect to durable phase advancement, desktop drag repositioning with persisted normalized coordinates, and an installed-package gate that binds acceptance to the exact validated ARM64 build by `.installed-build-sha`, PE machine `0xAA64` and checkout `HEAD`. The ARM64 updater now also keeps one shared RuntimeBundle and has a real Windows Junction cleanup regression test so frequent UI builds do not deep-copy `node_modules` or prompt while removing links. The M3 beginner product surface is deliberately narrowed to exactly one `玻璃时钟`, `今日待办`, and `玻璃天气` during acceptance, with preset-owned geometry, collision-safe automatic placement on create, and desktop drag for repositioning; resize/monitor-edit APIs remain deferred and guarded by `verify-widget-product-model.ps1` in both exact-head Windows workflows. The remaining gate is real ARM64 Widget visibility/icon-layer/Settings/Search/Explorer-restart/monitor-reconnect acceptance followed by same-session human visual attestation and sealed independent verification. M4 does not begin until that gate is satisfied.
