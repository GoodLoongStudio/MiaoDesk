# MiaoDesk Native 技术路线基线

- 状态：**已确认**
- 日期：2026-08-24
- 适用范围：MiaoDesk Native 主线
- 正式开发分支：`main`
- 产品基线：`docs/MIAODESK-PRODUCT-BASELINE.md`
...
```text
MiaoDesk
├─ A. 顶部统一入口：应用 / 文件 / 妙喵 AI
├─ B. Wallpaper Engine 级 Desktop Composition
└─ C. DeepSeek Harness 高级工作台
```

“Wallpaper Engine 级”只表示产品能力深度。**具体 Windows 壁纸运行时实现不得以 Wallpaper Engine 为工程参考**；统一以 `LIVELY_CPP_WALLPAPER_IMPLEMENTATION.md`、Lively 的公开行为和 Microsoft Windows API 文档为技术参考，并由 MiaoDesk 独立 C++23 实现。
...
4. Pi 运行在 MiaoDesk 自带 Node 24 上，不要求用户安装 Node/npm。
...
8. MiaoDesk 只保留真正依赖产品内部状态的 Desktop Tools。
...
14. Lively 是 GPL-3.0、MiaoDesk 是 MIT；只能研究行为和 API 序列，禁止复制或逐行翻译 Lively 源码。
...
API Key 只长期存 Windows Credential Manager。MiaoDesk Pi 目录：
...
当前第一阶段桥接接口包括：桌面状态读取、Web `.mdwall` 校验/应用、Widget CRUD。它们后续收口到版本化 Desktop Control API。
...
Lively public behavior + Microsoft APIs
  ↓
MiaoDesk DesktopShellHost / Surface Managers
  ↓
