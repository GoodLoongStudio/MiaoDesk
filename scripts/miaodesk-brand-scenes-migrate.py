...
WORKFLOW = ROOT / ".github" / "workflows" / "miaodesk-brand-scenes.yml"
...
PRODUCT_REPLACEMENTS = [
    ("Aurora Flow · 极光流动", "妙喵云境 · 云海星光"),
    ("Neon Flow · 霓虹网格", "霓虹之城 · 雨夜光轨"),
    ("Ocean Flow · 深海浪潮", "月影秘境 · 月湖萤火"),
    ("流动极光 · 夜空星幕", "云端妙喵 · 星光花瓣"),
    ("霓虹幻境 · 赛博公路", "未来都市 · 雨夜光轨"),
    ("深海浪潮 · 澄蓝海底", "月湖秘境 · 萤火薄雾"),
    ("Aurora Flow", "妙喵云境"),
    ("Neon Flow", "霓虹之城"),
    ("Ocean Flow", "月影秘境"),
    ("Aurora 动态壁纸", "妙喵云境动态壁纸"),
    ("回退 Aurora", "回退妙喵云境"),
    ("默认回退 Aurora", "默认回退妙喵云境"),
    ("Aurora / Neon / Ocean", "妙喵云境 / 霓虹之城 / 月影秘境"),
    ("PaintAurora", "PaintMiaoCloud"),
    ("PaintNeon", "PaintNeonCity"),
    ("PaintOcean", "PaintMysticMoon"),
    ("DrawAurora", "DrawMiaoCloud"),
    ("DrawNeon", "DrawNeonCity"),
    ("DrawGrid", "DrawMysticMoon"),
    ("FallbackSlotToAurora", "FallbackSlotToMiaoCloud"),
    ("FallbackToAurora", "FallbackToMiaoCloud"),
    (".tdwall", ".mdwall"),
    ("TDWALL", "MDWALL"),
]
...
TEXT_SUFFIXES = {
    ...
    ".md", ".txt", ".rst", ".json", ".jsonc", ".xml", ".manifest", ".rc", ".def",
    ".yml", ".yaml", ".toml", ".ini", ".cfg", ".conf", ".props", ".targets",
    ".cmake", ".ps1", ".psm1", ".psd1", ".cmd", ".bat", ".sh", ".py", ".js", ".ts",
    ".html", ".htm", ".css", ".scss", ".svg", ".cs", ".csproj", ".sln", ".vcxproj",
    ".filters", ".natvis", ".editorconfig", ".gitattributes", ".gitignore",
}
TEXT_NAMES = {"CMakeLists.txt", "LICENSE", "Dockerfile", "Makefile"}

SCENE_HEADER = r'''#pragma once
...
def rename_paths() -> int:
    changed = 0
    # Bottom-up so children are already stable when a parent directory moves.
    for current_root, dirs, files in os.walk(ROOT, topdown=False):
        ...
        for filename in files:
            src = root / filename
            new_name = replace_name(filename)
...
def patch_store_demo() -> None:
    path = ROOT / "src" / "native" / "src" / "desktop" / "demo" / "StoreDemoExperience.cpp"
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    text = text.replace(
        'L"ocean", L"海洋", L"海边", L"蓝", L"深海", L"海浪"',
        'L"ocean", L"海洋", L"海边", L"蓝", L"深海", L"海浪", L"mystic", L"moon", L"月影", L"秘境", L"森林", L"萤火"')
    text = text.replace('L"给我一个极光壁纸"', 'L"给我一个妙喵云境壁纸"')
    text = text.replace('L"极光壁纸"', 'L"妙喵云境壁纸"')
    path.write_text(text, encoding="utf-8")


def install_scene_renderer() -> None:
    path = ROOT / "src" / "native" / "include" / "miaodesk" / "SceneWallpaperPainter.h"
    if not path.exists():
        raise RuntimeError(f"scene renderer missing after rename: {path}")
    path.write_text(SCENE_HEADER, encoding="utf-8", newline="\n")
...
def main() -> None:
    text_changes = rewrite_text_files()
    path_changes = rename_paths()
    patch_store_demo()
    install_scene_renderer()
    remove_one_shot_files()
    print(f"MiaoDesk migration prepared: text_files={text_changes}, renamed_paths={path_changes}")
