...
- `pi/` — pinned production dependency tree of `@earendil-works/pi-coding-agent`; Pi is the default TuringDesk Agent Runtime.
...
Pi uses the TuringDesk-bundled Node runtime. On Windows its built-in shell tool is configured to use the system Windows PowerShell executable, so Git Bash is not an end-user prerequisite.
