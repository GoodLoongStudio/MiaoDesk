...
详细规范见 `PI_AGENT_CONVERSATION_UX.md` 与 `PI_AGENT_ACTIVITY_FEEDBACK.md`；视觉语言见 `TURINGDESK_GLASS_UI_DESIGN_LANGUAGE.md`。

### 4.3 工具能力

通用 Agent 能力由 Pi 管理：read / write / edit / grep / find / ls / shell / Skills / Extensions / Pi Packages。

TuringDesk 只保留真正属于桌面产品的专属工具（设置、壁纸预览、小组件列举、包校验）。详见第 7 节。

---

## 5. 原则 4 — DeepSeek Harness

TuringDesk 不 fork、不魔改 DeepSeek Harness。后台服务启动必须禁止自动打开外部浏览器：
...
Harness UI 只在用户明确点击时由 `TuringDeskHarness.exe` 的 WebView2 窗口显示。不回退到系统 Node、全局 npm、npx 或在线安装。
...
|---|---|
| `settings_open` | 打开设置页 |
| `wallpaper_validate_package` | 校验 .tdwall 包 |
...
| 可执行文件 | 职责 |
|---|---|
| `TuringDesk.exe` | Search / AI 对话 / 设置中心 |
| `TuringDeskWallpaper.exe` | 桌面壁纸引擎 + 小组件运行时 |
...
```
TURINGDESK-PRODUCT-BASELINE.md          <- 本文，唯一产品基线
...
`TURINGDESK_GLASS_UI_DESIGN_LANGUAGE.md`、`SEARCH_BAR_VISUAL_SPEC.md`。
