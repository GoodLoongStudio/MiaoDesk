...
function Resolve-BundleFile([string]$RelativePath) {
    if ([string]::IsNullOrWhiteSpace($RelativePath)) { throw 'Runtime V3 base manifest contains an empty archive path.' }
    $path = Join-Path $BundleRoot ($RelativePath -replace '/', '\')
    if (-not (Test-Path $path -PathType Leaf)) { throw "Runtime V3 base archive is missing: $path" }
    $path
}
...
function Assert-Hash([string]$Path, [string]$Expected) {
    ...
    if ([string]::IsNullOrWhiteSpace($Expected) -or $actual -ne $Expected.ToLowerInvariant()) {
        throw "Runtime V3 base integrity check failed: $Path`nExpected: $Expected`nActual:   $actual"
    }
}
...
function Expand-ArchiveWithTar([string]$Archive, [string]$Destination) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Unable to extract Runtime V3 base archive: $Archive" }
}

if (-not (Test-Path $ManifestPath -PathType Leaf) -or -not (Test-Path $CompleteMarker -PathType Leaf)) {
    throw 'Pinned x64 RuntimeBundle metadata is incomplete.'
}
$manifest = Get-Content $ManifestPath -Raw | ConvertFrom-Json
if ([string]$manifest.architecture -ne 'x64' -or [int]$manifest.schema -lt 2) {
    throw 'Pinned x64 RuntimeBundle manifest is incompatible.'
}

# Runtime V3 deliberately consumes only native/base runtime artifacts from the
# V2 repository bundle. The legacy DSH/Pi archives are not expanded at all.
...
$nodeTemp = Join-Path $tempBase ('MiaoDesk-V3-Node-' + [guid]::NewGuid().ToString('N'))
$gozTemp = Join-Path $tempBase ('MiaoDesk-V3-Goz-' + [guid]::NewGuid().ToString('N'))
try {
    Expand-ArchiveWithTar $nodeArchive $nodeTemp
    $nodeHome = Get-ChildItem $nodeTemp -Directory -Recurse -ErrorAction Stop |
    ...
    Expand-ArchiveWithTar $gozArchive $gozTemp
...
foreach ($required in @($nodeExe,$npmCmd,(Join-Path $gozDir 'goz.exe'),(Join-Path $gozDir 'gozd.exe'))) {
    if (-not (Test-Path $required -PathType Leaf)) { throw "Runtime V3 base staging is incomplete: $required" }
}

& $nodeExe --version | Out-Host
if ($LASTEXITCODE -ne 0) { throw 'Runtime V3 base Node probe failed.' }

Write-Host 'Runtime V3 x64 base ready: Node + npm + goz only; legacy DSH/Pi trees were never expanded.' -ForegroundColor Green
