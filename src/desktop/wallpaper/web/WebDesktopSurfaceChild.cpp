...
fs::path UserDataDirectory(std::wstring_view token) {
    ...
    return paths::EnsureDirectory(
        root.empty() ? fs::path{} : root / L"DesktopSurface" / SafeToken(std::wstring(token)));
}
...
private:
    ...
    // Injected into every web wallpaper so hand-authored content can observe audio.
    //
    // This constant must stay byte-identical to
...
(function () {
  ...
  window.wallpaper = {
    ...
    // Runs before any script the page defines, so window.wallpaper exists by the
    // time authored code looks for it. Failure is non-fatal: a page that never
    // registers a listener simply gets no frames, and the wallpaper still renders.
    ...
    void InstallAudioBridge() {
        if (!webview_) return;
        // A named local rather than a temporary's c_str(): the callee stores the
        // script, and a dangling pointer here would be a much later, much more
        // confusing failure. MultiByteToWideChar rather than a wstring(begin, end)
        // narrowing, which would corrupt the script the moment it contains non-ASCII.
        const std::wstring script = Utf8ToWide(kWebAudioBridgeScript);
