# MiaoDesk wallpaper capability parity roadmap

Status: current implementation roadmap. This document is subordinate to `MIAODESK-PRODUCT-BASELINE.md`; if they conflict, the product baseline wins.

Goal: evolve the native MiaoDesk desktop subsystem into a polished Wallpaper Engine-class desktop engine, with equivalent functional depth where useful, while keeping MiaoDesk branding, assets and implementation original. MiaoDesk adds two first-class differentiators: AI desktop control and persistent desktop widgets.
...
Lively is GPL-3.0 while MiaoDesk is MIT. MiaoDesk therefore uses a clean-room-style native C++ reimplementation: study behavior/API sequences and edge cases, then independently implement them. Do not copy or mechanically translate Lively source.
...
- Three native scenes: 妙喵云境, 霓虹之城, 月影秘境 (scene id `scene-grid`).
...
- MiaoDesk-owned UI windows are excluded from fullscreen/maximized wallpaper throttling decisions.
...
- `.mdwall` Web package creation and validation.
- Pi Agent can create validated Web `.mdwall` packages.
...
- Pi Agent desktop control interfaces for reading desktop state, applying validated Web `.mdwall` packages, and creating/updating/removing/listing widgets.
...
- widgets as an additional MiaoDesk layer, not as a substitute for wallpaper parity.
...
- [x] Keep MiaoDesk settings/search UI from accidentally pausing its own wallpaper runtime.
...
### P3 — advanced parity and MiaoDesk differentiation
...
- [ ] Portable `.mdwall` / `.tdwidget` import-export with metadata, versioning, hashes and safe extraction.

## AI contract

AI must never edit private INI files or internal runtime state directly as its long-term control model. The target architecture is a versioned MiaoDesk Desktop Control API:

```text
Pi Agent
  -> typed MiaoDesk desktop tools
  -> Desktop Control API
  -> validate mutation
  -> apply transaction
