...
function Get-MachineFingerprint($Report) {
    ...
    $parts = @(
        [string](Get-PropertyValue $machine 'computerName'),
        [string](Get-PropertyValue $machine 'windowsBuild'),
        [string](Get-PropertyValue $machine 'totalPhysicalMemoryBytes'),
        $cpu,
        $gpu,
        [string](Get-PropertyValue $Report 'logicalProcessors')
    )
    return ($parts -join '::')
}

function Add-PerformanceErrors(
    [string]$Scenario,
    [string]$Path,
    [System.Collections.Generic.List[string]]$Errors
) {
...
function Test-ManualSignoff(
    ...
    if ((Get-PropertyValue $signoff 'schema') -ne 1) {
        $Errors.Add('manual-signoff.json: schema must be 1.')
    }

    $candidateSha = [string](Get-PropertyValue $signoff 'candidateSha')
    if ($candidateSha -notmatch '^[0-9a-fA-F]{40}$') {
        $Errors.Add('manual-signoff.json: candidateSha must be a full 40-character Git commit SHA.')
    }
    if ([string]::IsNullOrWhiteSpace([string](Get-PropertyValue $signoff 'reviewer'))) {
        $Errors.Add('manual-signoff.json: reviewer is required.')
    }
    $reviewedAt = [string](Get-PropertyValue $signoff 'reviewedAt')
    $parsedDate = [DateTimeOffset]::MinValue
    if ([string]::IsNullOrWhiteSpace($reviewedAt) -or -not [DateTimeOffset]::TryParse($reviewedAt, [ref]$parsedDate)) {
        $Errors.Add('manual-signoff.json: reviewedAt must be a valid timestamp.')
    }

    $checks = Get-PropertyValue $signoff 'checks'
    $requiredChecks = @(
        'wallpaperComposition',
        'widgetAlphaZOrderInteraction',
        'noClippingLandscapePortrait',
        'mixedDpiDrag',
        'wallpaperOffWidgetsAlive',
        'explorerRestartRecovery',
        'sleepResumeRecovery',
        'cleanInstallerSmoke'
    )
    foreach ($name in $requiredChecks) {
        if ($null -eq $checks -or (Get-PropertyValue $checks $name) -ne $true) {
            $Errors.Add("manual-signoff.json: '$name' must be explicitly true after human verification.")
        }
    }
    return $signoff
}
...
function Test-EvidenceRoot([string]$Root) {
    ...
    if ([string]::IsNullOrWhiteSpace($Root) -or -not (Test-Path $Root -PathType Container)) {
        $errors.Add("Evidence root does not exist: $Root")
        return $errors
    }

    $visualInitialPath = Join-Path $Root 'visual/initial/acceptance.json'
    $visualExplorerPath = Join-Path $Root 'visual/explorer-restart/acceptance.json'
    $visualSleepPath = Join-Path $Root 'visual/sleep-resume/acceptance.json'

    $initial = Add-VisualErrors 'visual/initial' $visualInitialPath $true $errors
    $explorer = Add-VisualErrors 'visual/explorer-restart' $visualExplorerPath $false $errors
    $sleep = Add-VisualErrors 'visual/sleep-resume' $visualSleepPath $false $errors

    $visualMachines = @(
        $initial,
        $explorer,
        $sleep
    ) | Where-Object { $null -ne $_ } | ForEach-Object { [string](Get-PropertyValue $_ 'machine') } | Select-Object -Unique
    if ($visualMachines.Count -gt 1) {
        $errors.Add('Visual acceptance captures came from different machines.')
    }

    $performanceReports = @{}
    foreach ($scenario in @('desktop-only','wallpaper','widgets-3','ai-idle')) {
        $path = Join-Path $Root "performance/performance-$scenario.json"
        $performanceReports[$scenario] = Add-PerformanceErrors $scenario $path $errors
    }

    $fingerprints = @(
        $performanceReports.Values |
            Where-Object { $null -ne $_ } |
            ForEach-Object { Get-MachineFingerprint $_ } |
            Where-Object { -not [string]::IsNullOrWhiteSpace($_) } |
            Select-Object -Unique
    )
    if ($fingerprints.Count -gt 1) {
        $errors.Add('Performance scenarios do not share one hardware/OS fingerprint.')
    }

    if ($null -ne $initial) {
        $visualMachine = [string](Get-PropertyValue $initial 'machine')
        foreach ($scenario in $performanceReports.Keys) {
            $report = $performanceReports[$scenario]
...
function Write-SyntheticEvidence([string]$Root) {
    New-Item -ItemType Directory -Path (Join-Path $Root 'visual/initial') -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path $Root 'visual/explorer-restart') -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path $Root 'visual/sleep-resume') -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path $Root 'performance') -Force | Out-Null

    foreach ($slot in @('initial','explorer-restart','sleep-resume')) {
        $summary = [ordered]@{
            monitorCount=2
            mixedDpi=$true
            portraitMonitorCount=1
            surfaceCount=3
            widgetSurfaceCount=3
            wallpaperSurfaceCount=0
            processCount=2
            warningCount=0
        }
        $report = [ordered]@{
        ...
        $dir = Join-Path $Root "visual/$slot"
        $report | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $dir 'acceptance.json') -Encoding UTF8
        [IO.File]::WriteAllBytes((Join-Path $dir 'desktop.png'), [byte[]](1,2,3,4))
    }
    ...
    foreach ($scenario in @('desktop-only','wallpaper','widgets-3','ai-idle')) {
        $report = [ordered]@{
            ...
            logicalProcessors=8
            metrics=[ordered]@{
                averageCpuPercent=0.5
                p95CpuPercent=1.0
                peakCpuPercent=2.0
                averageWorkingSetBytes=104857600
                averagePrivateMemoryBytes=94371840
                averageHandles=120
                averageThreads=12
                averageProcessCount=3
            }
            comparison=$null
            samples=@(1,2,3,4,5,6)
        }
    ...
    $signoff = [ordered]@{
        ...
        checks=[ordered]@{
            wallpaperComposition=$true
            widgetAlphaZOrderInteraction=$true
            noClippingLandscapePortrait=$true
            mixedDpiDrag=$true
            wallpaperOffWidgetsAlive=$true
            explorerRestartRecovery=$true
            sleepResumeRecovery=$true
            cleanInstallerSmoke=$true
        }
    }
...
if ($SelfTest) {
    ...
    try {
        ...
        $errors = @(Test-EvidenceRoot $root)
        if ($errors.Count -ne 0) {
            throw ("Expected valid synthetic evidence to pass:" + [Environment]::NewLine + ($errors -join [Environment]::NewLine))
        }

        $badPath = Join-Path $root 'performance/performance-ai-idle.json'
        $bad = Get-Content $badPath -Raw | ConvertFrom-Json
        $bad.machine.computerName = 'OTHER-MACHINE'
        $bad | ConvertTo-Json -Depth 6 | Set-Content $badPath -Encoding UTF8
        $errors = @(Test-EvidenceRoot $root)
        if (-not ($errors -match 'hardware/OS fingerprint|does not match visual machine')) {
            throw 'Expected mixed-machine evidence injection to fail.'
        }

        Write-SyntheticEvidence $root
        $signoffPath = Join-Path $root 'manual-signoff.json'
        $signoff = Get-Content $signoffPath -Raw | ConvertFrom-Json
        $signoff.checks.cleanInstallerSmoke = $false
        $signoff | ConvertTo-Json -Depth 5 | Set-Content $signoffPath -Encoding UTF8
        $errors = @(Test-EvidenceRoot $root)
        if (-not ($errors -match 'cleanInstallerSmoke')) {
            throw 'Expected missing human sign-off injection to fail.'
        }

        Write-Host 'RC evidence verifier self-test passed.' -ForegroundColor Green
        exit 0
    }
    finally {
...
if ([string]::IsNullOrWhiteSpace($EvidenceRoot)) {
    throw 'Usage: verify-rc-evidence.ps1 -EvidenceRoot <folder>, or -SelfTest.'
}

$resolvedRoot = (Resolve-Path $EvidenceRoot).Path
$errors = @(Test-EvidenceRoot $resolvedRoot)
if ($errors.Count -gt 0) {
    foreach ($errorText in $errors) {
        Write-Host "::error::$errorText"
    }
    Write-Error "RC evidence verification failed with $($errors.Count) issue(s)."
    exit 1
}

Write-Host 'RC evidence structure and explicit human sign-off are complete.' -ForegroundColor Green
Write-Host "Evidence root: $resolvedRoot"
