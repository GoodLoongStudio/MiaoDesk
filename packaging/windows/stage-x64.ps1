...
& (Join-Path $PSScriptRoot 'prepare-runtime-base-x64.ps1') -DeployDir $Destination
if ($LASTEXITCODE -ne 0) { throw "Base Runtime staging failed with exit code $LASTEXITCODE" }

& (Join-Path $PSScriptRoot 'build-agent-runtime.ps1') -Root $Destination -Architecture x64
...
Write-Host "Windows x64 package staging ready: $Destination" -ForegroundColor Green
