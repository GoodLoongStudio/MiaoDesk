...
const PropertyDefinition* MiaoSceneRuntimeModel::FindProperty(
    ...
    for (const auto& property : component->properties) {
        if (property.name == address.propertyName) return &property;
    }
    return nullptr;
}
...
bool MiaoSceneRuntimeModel::Validate(const SceneRuntimeDefinition& runtime, std::wstring* error) {
    ...
    for (const auto& material : runtime.materials) {
        ...
        if (material.model == MaterialModel::Builtin) {
            if (material.builtinName.empty())
                return Fail(error, L"Builtin material requires builtinName: " + material.id);
        ...
        for (const auto& property : material.properties) {
            if (property.name.empty())
                return Fail(error, L"Material property name cannot be empty: " + material.id);
        ...
        for (const auto& texture : material.textures) {
            if (texture.slot.empty())
                return Fail(error, L"Material texture slot cannot be empty: " + material.id);
            ...
            if (!asset)
                return Fail(error, L"Material texture must reference an existing asset: " + material.id + L"/" + texture.slot);
    ...
    for (const auto& binding : runtime.bindings) {
        ...
        if (!bindingIds.emplace(binding.id).second)
            return Fail(error, L"Duplicate binding id: " + binding.id);
        ...
        if (!target)
            return Fail(error, L"Binding target does not resolve to a component property: " + binding.id);
        ...
        if (binding.sourceKind == BindingSourceKind::Parameter) {
            ...
            if (!source)
                return Fail(error, L"Binding parameter source does not exist: " + binding.id);
        } else {
            source = FindInput(runtime, binding.sourceId);
            if (!source)
                return Fail(error, L"Binding input source does not exist: " + binding.id);
        }
...
bool MiaoPropertyStore::Set(const PropertyAddress& address, PropertyValue value, std::wstring* error) {
    ...
    if (it == values_.end())
        return Fail(error, L"Unknown scene property: " + address.componentId + L"/" + address.propertyName);
...
void MiaoPropertyStore::Clear() noexcept {
    values_.clear();
}

std::size_t MiaoPropertyStore::Size() const noexcept {
    return values_.size();
}
...
bool MiaoSceneRuntimeModel::SelfTest() {
    ...
    scene.assets.push_back(AssetDefinition{
        L"asset://shader/self-test",
        AssetType::Shader,
        L"shaders/self-test.hlsl",
    });
    scene.shaders.push_back(ShaderDefinition{
        L"shader://self-test/pixel",
        ShaderStage::Pixel,
        L"asset://shader/self-test",
        "main",
        true,
    ...
    runtime.materials.push_back(MaterialDefinition{
        L"material://builtin/sprite",
        MaterialModel::Builtin,
        L"sprite",
        L"",
        L"",
    ...
    runtime.materials.push_back(MaterialDefinition{
        L"material://programmable/self-test",
        MaterialModel::Programmable,
        L"",
        L"",
        L"shader://self-test/pixel",
        {PropertyDefinition{L"strength", PropertyType::Float, 1.0}},
        {},
    });
